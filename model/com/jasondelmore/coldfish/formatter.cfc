...
<cfcomponent output="false">
		...
		<cfscript>
			...
			while (isdefined("line")) {
				if (getConfig().getShowLineNumbers()) {
                	...
                	variables.buffer.append("<span style='" & getStyle("LINENUMBER") & "'>" & linenumber & "</span>");
				}
				formatLine(line);
	...
	<cffunction name="getToolbarHTML" access="private" output="true">
		<cfargument name="code" type="string"/>
		<cfargument name="codesig" type="string"/>
		<cfset var toolbarHTML = ""/>
		<cfsavecontent variable="toolbarHTML">
	...
	<cffunction name="keywordsearch" access="private" hint="This function searches for keywords." output="false">
		...
		<cfset var keyword = thisLine.substring(javacast('int',i)).split('[\s(]')[1]/> <!--- search starting from current position --->
